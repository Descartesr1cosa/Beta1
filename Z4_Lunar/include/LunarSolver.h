...
struct NumInfo
{
    // Single-fluid ion/electron number density, nondimensional.
    double n_true{0.0};
    double ne_true{0.0};
    double ne_eff{0.0};
    double mhd_weight{0.0};
    double mhd_taper{0.0};
};

struct ResistiveEdgeEMFControl
{
    bool is_Lunar_resistance = false;
    bool use_implicit_lunar_resistance = false;
    int n_subcycles = 1;
        ...
        for (int ib = 0; ib < nb; ++ib)
        {
            ...
            for (int i = clo.i; i < chi.i; ++i)
                for (int j = clo.j; j < chi.j; ++j)
                    for (int k = clo.k; k < chi.k; ++k)
                    {
                        NumInfo num = Hall_Num_Limiter(UH(i, j, k, 0));
...
private:
    ...
    NumInfo Hall_Num_Limiter(double rho);
