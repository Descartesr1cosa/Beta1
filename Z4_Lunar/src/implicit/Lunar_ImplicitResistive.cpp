...
void LunarSolver::SetupImplicitResistiveDiffusion_()
{
    ...
    if (nglb > 0)
    {
        ...
        PetscCallAbort(PETSC_COMM_WORLD,
                       MatShellSetOperation(implicit_resistive_A_, MATOP_MULT,
                                            (void (*)(void))&LunarSolver::MatMultImplicitResistive_));
...
void LunarSolver::DestroyImplicitResistiveDiffusion_()
{
...
void LunarSolver::BuildImplicitResistiveEdgeDofMap_()
{
...
double LunarSolver::ImplicitResistiveEtaAtEdge_(const TOPO::EdgeLocalID &e) const
{
...
void LunarSolver::SnapshotImplicitResistiveBstar_()
{
    const int nb = fld_->num_blocks();
    for (int ib = 0; ib < nb; ++ib)
    {
...
void LunarSolver::RestoreImplicitResistiveBstar_()
{
    const int nb = fld_->num_blocks();
    for (int ib = 0; ib < nb; ++ib)
    {
...
void LunarSolver::UnpackVecToImplicitEres_(Vec X)
{
    ...
    lunar_bound_.Sync("Eres1form");
}

void LunarSolver::CalcImplicitDeltaJedgeFromDeltaB_()
{
    ...
    lunar_bound_.Sync("dJ");
}

void LunarSolver::PackImplicitJedgeToVec_(Vec v, const IdTriplet &fid_Jedge,
...
PetscErrorCode LunarSolver::MatMultImplicitResistive_(Mat A, Vec X, Vec Y)
{
    void *ctx = nullptr;
    PetscCall(MatShellGetContext(A, &ctx));
    auto *S = static_cast<LunarSolver *>(ctx);
    PetscCheck(S, PETSC_COMM_WORLD, PETSC_ERR_ARG_NULL,
               "Null LunarSolver context in implicit resistive MatShell.");
...
// 做一次 Lunar 内部电阻磁扩散的隐式修正
void LunarSolver::SolveImplicitResistiveDiffusion_(double dt_step)
{
    if (!resist_control.is_Lunar_resistance)
        return;
    ...
    if (control_.if_outres)
    {
        ...
        if (par_->GetInt("myid") == 0)
        {
            ...
            std::cout << "[ImplicitLunarResistive] reason=" << reason
                      << " its=" << its
                      << " rnorm=" << rnorm
...
void LunarSolver::ApplyImplicitResistiveUpdate_(double dt_step)
{
    lunar_bound_.Sync("Eres1form");
