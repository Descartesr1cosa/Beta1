...
void LunarSolver::Debug_TestJOperator_Manufactured(int test_id)
{
    ...
    auto diagnose_Jcell_weights = [&](const std::string &mode_label)
    {
        ...
        int ibx_all = -1, ix_all = 0, jx_all = 0, kx_all = 0;
        int iby_all = -1, iy_all = 0, jy_all = 0, ky_all = 0;
        int ibz_all = -1, iz_all = 0, jz_all = 0, kz_all = 0;

        int ibx_safe = -1, ix_safe = 0, jx_safe = 0, kx_safe = 0;
        int iby_safe = -1, iy_safe = 0, jy_safe = 0, ky_safe = 0;
        int ibz_safe = -1, iz_safe = 0, jz_safe = 0, kz_safe = 0;
            ...
            for (int i = lo.i; i < hi.i; ++i)
                for (int j = lo.j; j < hi.j; ++j)
                    for (int k = lo.k; k < hi.k; ++k)
                    {
                        ...
                        for (int n = 0; n < 12; ++n)
                        {
                            sx += std::abs(W(i, j, k, n));
                            sy += std::abs(W(i, j, k, 12 + n));
                            sz += std::abs(W(i, j, k, 24 + n));
                        }
        ...
        out << "  ALL  max sum|W_Jx| = " << maxWx_all
            << " block=" << ibx_all << " idx=(" << ix_all << "," << jx_all << "," << kx_all << ")\n";
        out << "  ALL  max sum|W_Jy| = " << maxWy_all
            << " block=" << iby_all << " idx=(" << iy_all << "," << jy_all << "," << ky_all << ")\n";
        out << "  ALL  max sum|W_Jz| = " << maxWz_all
            << " block=" << ibz_all << " idx=(" << iz_all << "," << jz_all << "," << kz_all << ")\n";

        out << "  SAFE max sum|W_Jx| = " << maxWx_safe
            << " block=" << ibx_safe << " idx=(" << ix_safe << "," << jx_safe << "," << kx_safe << ")\n";
        out << "  SAFE max sum|W_Jy| = " << maxWy_safe
