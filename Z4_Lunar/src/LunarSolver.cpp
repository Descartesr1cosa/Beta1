...
#endif
{
    ...
    // ---- Build IO Module ----
    constexpr int NRES = 8; // single-fluid conserved variables plus induced magnetic field
    ...
    resist_control.is_Lunar_resistance = par_->GetBoo("is_Lunar_resistance");
    resist_control.use_implicit_lunar_resistance = par_->GetBoo("use_implicit_lunar_resistance");
    ...
    if (resist_control.use_implicit_lunar_resistance)
        SetupImplicitResistiveDiffusion_();

#ifdef HALL_IMPLICIT
    if (!topo_equiv_ || !edge_owner_pat_)
        throw std::runtime_error("LunarSolver: hall implicit topology/pattern is null.");

    hall_implicit_.Setup(grd_, topo_, fld_, halo_, par_, &lunar_bound_,
    ...
    cb.sync_dEedge = [this]()
    {
        lunar_bound_.Sync("dE");
    };

    cb.sync_dBface = [this]()
    {
        lunar_bound_.Sync("dB");
    };
    ...
    cb.sync_dJcell = [this]()
    {
        lunar_bound_.Sync("dJcell");
    };
    cb.sync_dEface = [this]()
    {
        lunar_bound_.Sync("Eface");
    };
