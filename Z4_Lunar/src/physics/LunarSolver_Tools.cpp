...
NumInfo LunarSolver::Hall_Num_Limiter(double rho)
{
    NumInfo nd;
    nd.n_true = std::max(rho, 0.0) / M_H * M_ref;
    nd.ne_true = nd.n_true;
    nd.ne_eff = std::sqrt(nd.ne_true * nd.ne_true + ne_hall_floor * ne_hall_floor);
    nd.mhd_weight = nd.n_true / nd.ne_eff;
    nd.mhd_taper = nd.mhd_weight;
    return nd;
}
...
void LunarSolver::calc_Uplus()
{
    ...
    for (int ib = 0; ib < nb; ++ib)
    {
        ...
        if (!Up.is_allocated() || !UH.is_allocated() || !PVH.is_allocated())
            continue;
        ...
        for (int i = lo.i; i < hi.i; ++i)
            for (int j = lo.j; j < hi.j; ++j)
                for (int k = lo.k; k < hi.k; ++k)
                {
                    const bool has_plasma = UH(i, j, k, 0) > rho_eps;
                    Up(i, j, k, 0) = has_plasma ? PVH(i, j, k, 0) : 0.0;
                    Up(i, j, k, 1) = has_plasma ? PVH(i, j, k, 1) : 0.0;
                    Up(i, j, k, 2) = has_plasma ? PVH(i, j, k, 2) : 0.0;
                }
    }

    lunar_bound_.Sync("Uplus");
}
