...
void LunarSolver::Scheme_U_()
{
    ...
    // Single fluid advection.
    do_one_species(fid_.fid_U_H, fid_.fid_PV_H, fid_.fid_RHS_H);
}

#if 0
// Retained only as a disabled reference while deriving the lunar single-fluid model.
...
// Single-fluid electromagnetic coupling. There is no Na photoionization,
// neutral drag, or inter-species energy exchange in the lunar model.
void LunarSolver::AddSourceToRHS_Fluid()
{
    const int nb = fld_->num_blocks();
    for (int ib = 0; ib < nb; ++ib)
    {
        ...
        for (int i = lo.i; i < hi.i; ++i)
            for (int j = lo.j; j < hi.j; ++j)
                for (int k = lo.k; k < hi.k; ++k)
                {
                    const NumInfo num = Hall_Num_Limiter(U(i, j, k, 0));
                    const double force_coeff = momentum_hall_coeff * num.mhd_weight;

                    const double jxb_x = J(i, j, k, 1) * B(i, j, k, 2) -
                                         J(i, j, k, 2) * B(i, j, k, 1);
                    const double jxb_y = J(i, j, k, 2) * B(i, j, k, 0) -
                                         J(i, j, k, 0) * B(i, j, k, 2);
                    const double jxb_z = J(i, j, k, 0) * B(i, j, k, 1) -
                                         J(i, j, k, 1) * B(i, j, k, 0);

                    RHS(i, j, k, 1) += force_coeff * jxb_x;
                    RHS(i, j, k, 2) += force_coeff * jxb_y;
                    RHS(i, j, k, 3) += force_coeff * jxb_z;
                    RHS(i, j, k, 4) += force_coeff *
                                        (jxb_x * PV(i, j, k, 0) +
                                         jxb_y * PV(i, j, k, 1) +
                                         jxb_z * PV(i, j, k, 2));
                }
